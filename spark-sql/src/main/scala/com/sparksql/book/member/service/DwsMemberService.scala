package com.sparksql.book.member.service

import com.sparksql.book.member.bean.{DwsMember, DwsMember_Result, MemberZipper, MemberZipperResult}
import com.sparksql.book.member.dao.DwdMemberDao
...
// dwdMember.join(dwdMemberRegtype,dwdMember("uid") === dwdMemberRegtype("uid")&&dwdMember("dn") === dwdMemberRegtype("dn"))
  ...
  def importMember(sparkSession: SparkSession, time: String) = { //隐式转换
    import sparkSession.implicits._ //隐式转换
    //查询全量数据 刷新到宽表
