package com.sparksql.book.qz.dao
...
object AdsQzDao {
  ...
  def getAvgSPendTimeAndScore(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"""select
                        ...
                        |order by score desc,spendtime desc""".stripMargin)
  ...
  def getPaperScoreSegmentUser(sparkSession: SparkSession, dt: String) = {
    sparkSession.sql(s"""select
                        ...
                        |from
                        |    (
                        |     select paperviewid,paperviewname,userid,
                        ...
                        |order by paperviewid,score_segment""".stripMargin)
