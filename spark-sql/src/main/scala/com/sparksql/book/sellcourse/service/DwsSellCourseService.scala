package com.sparksql.book.sellcourse.service

import com.sparksql.book.sellcourse.bean.{DwdCourseShoppingCart, DwdSaleCourse}
import com.sparksql.book.sellcourse.dao.DwdSellCourseDao
...
object DwsSellCourseService {
  ...
  def importSellCourseDetail2(sparkSession: SparkSession, dt: String) = {
    ...
    val dwdSaleCourse = DwdSellCourseDao.getDwdSaleCourse(sparkSession).where(s"dt='${dt}'")
    val dwdCourseShoppingCart: DataFrame = DwdSellCourseDao.getDwdCourseShoppingCart(sparkSession).where(s"dt='${dt}'")
      .withColumnRenamed("discount", "cart_discount")
      .withColumnRenamed("createtime", "cart_createtime")
    val dwdCoursePay = DwdSellCourseDao.getDwdCoursePay(sparkSession).where(s"dt='${dt}'")
      .withColumnRenamed("discount", "pay_discount")
...
//    val value: Dataset[Row] = broadcast(dwdSaleCourse)
//    value.cache()
  ...
  def importSellCourseDetail4(sparkSession: SparkSession, dt: String) = {
    ...
    val dwdSaleCourse = DwdSellCourseDao.getDwdSaleCourse(sparkSession).where(s"dt='${dt}'")
    val dwdCourseShoppingCart: DataFrame = DwdSellCourseDao.getDwdCourseShoppingCart2(sparkSession).where(s"dt='${dt}'")
      .drop("coursename")
      .withColumnRenamed("discount", "cart_discount")
      .withColumnRenamed("createtime", "cart_createtime")
