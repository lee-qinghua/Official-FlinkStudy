...
-- 前10s的总和
create view one_day_before_amount as
select
        et,
        sum(every_type_info_table.xiaofei_amount) over (order by et RANGE BETWEEN INTERVAL '10' second preceding and CURRENT ROW) as one_xiaofei_amount,
        sum(every_type_info_table.bangong_amount) over (order by et range between interval '10' second preceding and current row) as one_bangong_amount,
        sum(every_type_info_table.xinzi_amount) over (order by et range between interval '10' second preceding and current row) as one_xinzi_amount,
        sum(every_type_info_table.baoxian_amount) over (order by et range between interval '10' second preceding and current row) as one_baoxian_amount,
        sum(every_type_info_table.touzi_amount) over (order by et range between interval '10' second preceding and current row) as one_touzi_amount,
        sum(every_type_info_table.rongzi_amount) over (order by et range between interval '10' second preceding and current row) as one_rongzi_amount,
        sum(every_type_info_table.zhuanzhang_amount) over (order by et range between interval '10' second preceding and current row) as one_zhuanzhang_amount,
        sum(every_type_info_table.cunqu_amount) over (order by et range between interval '10' second preceding and current row) as one_cunqu_amount,
        sum(every_type_info_table.other_amount) over (order by et range between interval '10' second preceding  and current row) as one_other_amount
from every_type_info_table;


-- 前20s的总和
create view two_day_before_amount as
select
        ts,
        sum(every_type_info_table.xiaofei_amount) over (order by ts range between interval '20' second and current row) as two_xiaofei_amount,
        sum(every_type_info_table.bangong_amount) over (order by ts range between interval '20' second and current row) as two_bangong_amount,
        sum(every_type_info_table.xinzi_amount) over (order by ts range between interval '20' second and current row) as two_xinzi_amount,
        sum(every_type_info_table.baoxian_amount) over (order by ts range between interval '20' second and current row) as two_baoxian_amount,
        sum(every_type_info_table.touzi_amount) over (order by ts range between interval '20' second and current row) as two_touzi_amount,
        sum(every_type_info_table.rongzi_amount) over (order by ts range between interval '20' second and current row) as two_rongzi_amount,
        sum(every_type_info_table.zhuanzhang_amount) over (order by ts range between interval '20' second and current row) as two_zhuanzhang_amount,
        sum(every_type_info_table.qunqu_amount) over (order by ts range between interval '20' second and current row) as two_cunqu_amount,
        sum(every_type_info_table.other_amount) over (order by ts range between interval '20' second and current row) as two_other_amount
from every_type_info_table;


-- 前30s的总和
create view three_day_before_amount as
select
        ts,
        sum(every_type_info_table.xiaofei_amount) over (order by ts range between interval '20' second and current row) as three_xiaofei_amount,
        sum(every_type_info_table.bangong_amount) over (order by ts range between interval '20' second and current row) as three_bangong_amount,
        sum(every_type_info_table.xinzi_amount) over (order by ts range between interval '20' second and current row) as three_xinzi_amount,
        sum(every_type_info_table.baoxian_amount) over (order by ts range between interval '20' second and current row) as three_baoxian_amount,
        sum(every_type_info_table.touzi_amount) over (order by ts range between interval '20' second and current row) as three_touzi_amount,
        sum(every_type_info_table.rongzi_amount) over (order by ts range between interval '20' second and current row) as three_rongzi_amount,
        sum(every_type_info_table.zhuanzhang_amount) over (order by ts range between interval '20' second and current row) as three_zhuanzhang_amount,
        sum(every_type_info_table.qunqu_amount) over (order by ts range between interval '20' second and current row) as three_cunqu_amount,
        sum(every_type_info_table.other_amount) over (order by ts range between interval '20' second and current row) as three_other_amount
from every_type_info_table;
